...
clean_sql_file_path = '/opt/airflow/dags/sql/clean_and_move_data.sql'

# Read the SQL file into a variable
with open(clean_sql_file_path, 'r') as f:
    clean_sql = f.read()

# Define the configuration for BigQuery Job
configuration = {
    "query": {
        "query": clean_sql,  # Read SQL from the file
...
# Define the DAG
with models.DAG(
    'clean_and_move_to_final_table',
    default_args={'owner': 'airflow', 'start_date': days_ago(1)},
    schedule_interval=None,
) as dag:

    # Task: Execute SQL to clean the data and move to the final table
    clean_and_move_data = BigQueryInsertJobOperator(
        task_id='clean_and_move_data_to_final_table',
